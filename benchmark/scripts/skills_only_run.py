#!/usr/bin/env python3
"""
skills_only_run.py - Run the benchmark with the MetaClaw proxy in skills-only mode.

The proxy serves pre-built skills to the agent (no memory, no RL).  A fresh
copy of the skills directory is made before each run so the initial state is
always consistent.
...
# ===================== Configuration =====================
class cfg:
    LOG_FILE      = str(_METACLAW_ROOT / "benchmark" / "logs" / "skills_only_run" / "bench_run.log")
    ...
    PROXY_CONFIG  = str(_SCRIPT_DIR / "config" / "skills-only.yaml")
    # Set METACLAW_SKILLS_DIR to the directory containing your pre-built skills.
...
def start_proxy(env: dict, config_path: str, port: int) -> subprocess.Popen:
    ...
    print(f"[proxy] Starting proxy in skills-only mode (port={port})...")
...
def main():
    ...
    # Copy skills to a temp dir so each run starts from the same initial state.
